Shader "MMW/SquareBeatShader"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
        _SquareSpeed("Square Scroll Speed", Range(-200.0, 200.0)) = 2.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                v2f o;
                half timeVal = frac(_SquareSpeed*_Time.y);
                half timeValCalced = -(timeVal-1)*(timeVal-1)+1;
                o.vertex = UnityObjectToClipPos(v.vertex *(1+_VertexLength*timeValCalced));
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                half2 polarUV;
                half timeVal = _Time.y;
                half beatOneMinus = 1-_BeatLength;
                polarUV.x = frac(_ChangingSpeed * _Time.x);
                polarUV.y = frac(max(abs(val.x),abs(val.y))*beatOneMinus*2)- (frac(_SquareSpeed*_Time.y)-1)*_BeatLength; 
